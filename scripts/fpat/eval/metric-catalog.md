...
| `labelFieldSync` (itemsChecked/byKind/mismatches/missingFieldValues; entries carry `kind`) | `board-consistency.json` → `metrics.labelFieldSync` | `metrics.boardConsistency.labelFieldSync` | degraded-null; per-kind split (issue vs PR, #87) is additive-optional and measured from post-baseline reports only — the frozen cycle-0 reference remains total-only; the split is the data for the parked PR-event-sync decision, never the decision itself |
